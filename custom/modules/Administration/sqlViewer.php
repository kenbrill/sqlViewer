<?php
//todo: remove all _POST['sql'] and run through something
...
$ss = new Sugar_Smarty();
$sqlFormatter = new SqlFormatter_sqlViewer();

//If the form was submitted
if (isset($_POST['submit']) && !empty($_POST['submit'])) {
    $submit = $_POST['submit'];
    switch ($submit) {
        case 'Retrieve SQL from LOG':
            if (isset($_POST['rows']) && !empty($_POST['rows']) && is_numeric($_POST['rows'])) {
                $rows = $_POST['rows'];
            } else {
                $rows = 500;
            }
            $queries = array();
            $log = file($sugar_config['log_file']);
            $index = 1;
            foreach ($log as $line) {
                if (stristr($line, 'Query:') !== false) {
                    $index++;
                    list($date, $line2) = explode('[', $line, 2);
                    list($trash, $query) = explode('Query: ', $line2, 2);
                    $query = trim($query);
                    $date = trim($date);
                    $md5 = md5($query);
                    $dateString = strtotime($date);
                    $urlQuery = htmlspecialchars($query);
                    $queries[$md5] = array('index'      => $index,
            ...
            $ss->assign('COLS', array(
                array('name' => 'Select', 'size' => '10%'),
                array('name' => 'Date Run', 'size' => '10%'),
                array('name' => 'Query', 'size' => '80%')));
            $ss->assign('TYPE', 'LIST');
            $ss->assign('DATA', $queries);
            break;
        case 'Submit and Run Query':
            $initialSQL = htmlspecialchars_decode(trim($_POST['sql']), ENT_QUOTES);
            if(checkRunSecurity($initialSQL)===1) {
                $finalSQL = outputQuery($initialSQL);
                $result = $GLOBALS['db']->query($finalSQL);
                $headers = array();
                $sqlData = array();
                while ($hash = $GLOBALS['db']->fetchByAssoc($result)) {
                    if (empty($headers)) {
                        $headers = array_keys($hash);
                    }
                    $sqlData[] = array_values($hash);
                }
                if (empty($sqlData)) {
                    $ss->assign('ERROR', "<h3>Empty result set, no rows returned.</h3>");
                }

                $ss->assign('COLS', $headers);
                $ss->assign('DATA', $sqlData);
                $ss->assign('RETURN_QUERY', $_POST['sql']);
                $ss->assign('TYPE', 'QUERY');
            } else {
                $ss->assign('ERROR', "<h3>Only single SELECT queries can be run.</h3>");
            }
            break;
        default:
            $initialSQL = htmlspecialchars_decode(trim($_POST['sql']), ENT_QUOTES);
    ...
    //if the form was requested for the first time
    $initialSQL = htmlspecialchars_decode(trim($_POST['sql']), ENT_QUOTES);
    $finalSQL = outputQuery($initialSQL);
    if ($finalSQL[0] === '*') {
        $run = 0;
...
if ((int)$run === 1) {
    $run = checkRunSecurity($initialSQL);
}
$finalSQL = str_replace('\n', '', $finalSQL);
$outputSQL = $sqlFormatter::format($finalSQL);
$ss->assign('ALLOW_RUN', $run);
$ss->assign('SQL', $initialSQL);
$ss->assign('FINAL_SQL', $outputSQL);
$ss->display('custom/modules/Administration/sqlViewer.tpl');

/**
 * Checks to see if its safe to run a query
 * @param string $sql
 * @return int
 */
function checkRunSecurity(string $sql): int
{
    $sql = trim($sql);

    //If there is a semi-colon at the end of the SQL then remove it
    if (substr($sql, -1) === ';') {
        $sql = substr($sql, 0, -1);
    }

    //If there are other semi-colons in the SQL then its multiple queries and
    // thus is forbidden.
    if (empty($sql) || strpos($sql, ';') !== false) {
        return 0;
    }

    //If its not a SELECT query then it is forbidden
 ...
 * @return string
 */
function outputQuery($initialSQL): string
{
    $queryType = 0;
    $finalSQL = '';
    $run = '';
    if (stristr($initialSQL, 'Types:') !== false) {
        $remove = array("\r", '"', '{', '}', '\n', PHP_EOL);
    } else {
        $remove = array("\r", '{', '}', '[', ']', '\n', PHP_EOL);
    }
    $preparedSQL = str_replace($remove, '', $initialSQL);
    list($sql, $temp) = explode('Params:', $preparedSQL);
    list($params, $types) = explode('Types:', $temp);
    if (empty($types)) $queryType = 1;
    $paramsParts = explode(',', $params);
    $typeArray = array();
    $paramsArray = array();
    foreach ($paramsParts as $paramsPart) {
        if ($queryType === 0) {
            $temp = explode(':', $paramsPart, 2);
            $paramsArray[(int)$temp[0]] = trim($temp[1]);
        } else {
            $paramsArray[] = trim(str_replace('"', "'", $paramsPart));
        }
    }
    if ($queryType === 0) {
        $typeParts = explode(',', $types);
        foreach ($typeParts as $typePart) {
            $temp = explode(':', $typePart);
            $typeArray[(int)$temp[0]] = trim($temp[1]);
        }
    }
    $sqlArray = str_split($sql);
    $currentIndex = 1 - $queryType;
    foreach ($sqlArray as $char) {
        if ($char === '?') {
            if ($queryType === 0) {
                switch ($typeArray[$currentIndex]) {
                    case 5:
 ...
 *
 * I renamed the class to keep it from colliding with other instances of this class that
 * may exist.
 *
